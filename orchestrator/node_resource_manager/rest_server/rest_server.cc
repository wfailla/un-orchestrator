...
bool RestServer::parseGraph(Value value, highlevel::Graph &graph,
		bool newGraph) {
	...
	try {
		...
		for (Object::const_iterator i = obj.begin(); i != obj.end(); ++i) {
			...
			if (name == FORWARDING_GRAPH) {
				...
				try {
					...
					logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__,
							"The content does not respect the JSON syntax: \"%s\" should be an Object",
							FORWARDING_GRAPH);
					return false;
				}
				for (Object::const_iterator fg = forwarding_graph.begin();
						fg != forwarding_graph.end(); fg++) {
					bool e_if = false, e_vlan = false;
#if 0
					bool e_if_out = false
#endif

					string id, v_id, node, iface, e_name, node_id, sw_id,
							interface;
					...
					if (fg_name == _ID)
						logger(ORCH_DEBUG, MODULE_NAME, __FILE__, __LINE__,
								"\"%s\"->\"%s\": \"%s\"", FORWARDING_GRAPH, _ID,
								fg_value.getString().c_str());
					else if (fg_name == _NAME) {
						logger(ORCH_DEBUG, MODULE_NAME, __FILE__, __LINE__,
								"\"%s\"->\"%s\": \"%s\"", FORWARDING_GRAPH,
								_NAME, fg_value.getString().c_str());
						...
						logger(ORCH_DEBUG, MODULE_NAME, __FILE__, __LINE__,
								"\"%s\"->\"%s\": \"%s\"", FORWARDING_GRAPH,
								F_DESCR, fg_value.getString().c_str());
					...
					//Identify the VNFs
						...
						try {
							try {
								...
								logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__,
								__LINE__,
										"The content does not respect the JSON syntax: \"%s\" should be an Array",
										VNFS);
								return false;
							}
							...
							for (unsigned int vnf = 0; vnf < vnfs_array.size();
									++vnf) {
								try {
									vnfs_array[vnf].getObject();
								} catch (exception& e) {
									logger(ORCH_DEBUG_INFO, MODULE_NAME,
									__FILE__, __LINE__,
											"The content does not respect the JSON syntax: \"%s\" element should be an Object",
											VNFS);
									return false;
								}
								...
								string id, name, vnf_template, groups, port_id,
										port_name;
								...
								for (Object::const_iterator nf =
										network_function.begin();
										nf != network_function.end(); nf++) {
									const string& nf_name = nf->first;
									const Value& nf_value = nf->second;

									if (nf_name == _NAME) {
										logger(ORCH_DEBUG, MODULE_NAME,
										__FILE__, __LINE__,
												"\"%s\"->\"%s\": \"%s\"", VNFS,
												_NAME,
												nf_value.getString().c_str());
										foundName = true;
										if (!graph.addNetworkFunction(
												nf_value.getString())) {
											logger(ORCH_WARNING, MODULE_NAME,
											__FILE__, __LINE__,
													"Two VNFs with the same name \"%s\" in \"%s\"",
													nf_value.getString().c_str(),
													VNFS);
											return false;
										}
										...
										logger(ORCH_WARNING, MODULE_NAME,
										__FILE__, __LINE__,
												"Key \"%s\" found. It is ignored in the current implementation of the %s",
												VNF_TEMPLATE, MODULE_NAME);
									} else if (nf_name == _ID) {
										logger(ORCH_DEBUG, MODULE_NAME,
										__FILE__, __LINE__,
												"\"%s\"->\"%s\": \"%s\"", VNFS,
										...
										logger(ORCH_WARNING, MODULE_NAME,
										__FILE__, __LINE__,
												"Key \"%s\" is ignored in this configuration of the %s!",
												UNIFY_CONTROL, MODULE_NAME);
										continue;
#else
										try {
											nf_value.getArray();
										...
										for (unsigned int ports = 0;
											...
											vnf_port_config[ports + 1] =
													port_config;

											//Add NF ports descriptions
											if (!graph.addNetworkFunctionPortConfiguration(
													name, vnf_port_config)) {
												logger(ORCH_WARNING,
												MODULE_NAME, __FILE__,
												__LINE__,
														"Two VNFs with the same name \"%s\" in \"%s\"",
								...
								if (!foundName) {
									logger(ORCH_DEBUG_INFO, MODULE_NAME,
									__FILE__, __LINE__,
											"Key \"%s\" not found in an element of \"%s\"",
											_NAME, VNFS);
									return false;
								}
...
#else
								highlevel::VNFs vnfs(id, name, groups,
										vnf_template, portS);
#endif
								graph.addVNF(vnfs);
								portS.clear();
							...
							logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__,
							__LINE__,
									"The \"%s\" element does not respect the JSON syntax: \"%s\"",
									VNFS, e.what());
							return false;
						}
					}
					//Identify the end-points
						...
						try {
							...
							for (unsigned int ep = 0;
									ep < end_points_array.size(); ++ep) {
								try {
								...
								for (Object::const_iterator aep =
										end_points.begin();
										aep != end_points.end(); aep++) {
									...
									if (ep_name == _ID) {
										...
										logger(ORCH_WARNING, MODULE_NAME,
										__FILE__, __LINE__,
												"Element \"%s\" is ignored by the current implementation of the %s",
												EP_REM, MODULE_NAME);
										//XXX: currently, this information is ignored
									...
									//identify interface end-points
										...
										for (Object::const_iterator epi =
												ep_iface.begin();
												epi != ep_iface.end(); epi++) {
										...
										logger(ORCH_WARNING, MODULE_NAME,
										__FILE__, __LINE__,
												"Element \"%s\" is ignored by the current implementation of the %s. This type of end-point is not supported!",
												EP_INTERNAL, MODULE_NAME);
									}
									//identify interface-out end-points
										...
										vlan_id[id] = make_pair(v_id,
												interface);
										logger(ORCH_DEBUG, MODULE_NAME,
										__FILE__, __LINE__,
												"\"%s\"->\"%s\":\"%s\"",
												id.c_str(),
												vlan_id[id].first.c_str(),
												vlan_id[id].second.c_str());
										...
										aep2++;
										for (; aep2 != end_points.end();
												aep2++) {
											const string& ep2_name = aep2->first;
											const Value& ep2_value =
													aep2->second;
											if (ep2_name == _ID) {
										...
										try {
											...
											for (Object::const_iterator epi =
													...
													epi != ep_gre.end();
													epi++) {
												const string& epi_name =
														epi->first;
												const Value& epi_value =
														epi->second;
											...
											highlevel::EndPointGre ep_gre(id,
													e_name, local_ip, remote_ip,
													interface, gre_key, ttl,
													safe);
								...
								if (e_if) {
									...
									highlevel::EndPointInterface ep_if(id,
											e_name, node_id, sw_id, interface);
								...
								//add vlan end-points
									...
									//FIXME: node_id and sw_id should be ignored
									highlevel::EndPointVlan ep_vlan(id, e_name,
											v_id, node_id, sw_id, interface);
						...
						logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__,
								"Invalid key \"%s\" in \"%s\"", fg_name.c_str(),
								FORWARDING_GRAPH);
						return false;
					}
				...
				for (Object::const_iterator bs = big_switch.begin();
						bs != big_switch.end(); bs++) {
					...
					if (bs_name == FLOW_RULES) {
						...
						try {
							...
							if (flow_rules_array.size() == 0) {
								logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__,
								__LINE__, "Key \"%s\" without rules",
								FLOW_RULES);
								return false;
							}
#endif
							//Itearate on the flow rules
							for (unsigned int fr = 0;
									fr < flow_rules_array.size(); ++fr) {
								//This is a rule, with a match, an action, and an ID
								Object flow_rule =
										flow_rules_array[fr].getObject();
								...
								//Parse the rule
								for (Object::const_iterator afr =
										flow_rule.begin();
										afr != flow_rule.end(); afr++) {
									...
									if (fr_name == _ID) {
										...
										try {
											foundMatch = true;
											if (!MatchParser::parseMatch(
													fr_value.getObject(), match,
													(*action), nfs_ports_found,
													nfs_id, iface_id,
													iface_out_id, vlan_id,
													gre_id, graph)) {
												return false;
											}
										} catch (exception& e) {
											logger(ORCH_DEBUG_INFO, MODULE_NAME,
											__FILE__, __LINE__,
													"The content does not respect the JSON syntax: \"%s\"",
													MATCH);
											return false;
										}
										...
										try {
											...
											const Array& actions_array =
													fr_value.getArray();

											enum port_type {
												VNF_PORT_TYPE, EP_PORT_TYPE
											};
											...
											for (unsigned int ac = 0;
													ac < actions_array.size();
													++ac) {
												foundAction = true;
												try {
												...
												if (theAction.size() != 1) {
													...
													MODULE_NAME,
													__FILE__, __LINE__,
															"Too many keywords in an element of \"%s\"",
															ACTIONS);
													return false;
												}

												for (Object::const_iterator a =
														theAction.begin();
														a != theAction.end();
														a++) {
													const string& a_name =
													...
													if (a_name == OUTPUT) {
														...
														if (foundOneOutputToPort) {
															logger(
																	...
																	__FILE__,
																	__LINE__,
																	"Only one between keys \"%s\", \"%s\" and \"%s\" are allowed in \"%s\"",
																	PORT_IN,
																	VNF,
																	ENDPOINT,
																	ACTIONS);
														...
														if (p_type
															...
															if (name == ""
																...
																logger(
																		...
																		__FILE__,
																		__LINE__,
																		"Network function \"%s\" is not valid. It must be in the form \"name:port\"",
																		vnf_name.c_str());
																return false;
															}
															...
															action =
																	new highlevel::ActionNetworkFunction(
																			name,
																			string(
																					port_in_name_tmp),
																			port);
															...
															if (nfs_ports_found.count(
																	name) != 0)
																ports_found =
																		nfs_ports_found[name];
															ports_found.insert(
																	port);
															nfs_ports_found[name] =
																	ports_found;
														...
														//end-points port type
															...
															//vlan endpoint
																...
																actionType =
																		ACTION_ENDPOINT_VLAN;

																sscanf(
																		vlan_id[eP].first.c_str(),
																		"%u",
																		&vlanID);
															...
															//gre-tunnel endpoint
																...
																if (endPoint
																	...
																	logger(
																			...
																			__FILE__,
																			__LINE__,
																			"Graph end point \"%s\" is not valid. It must be in the form \"graphID:endpoint\"",
																			value.getString().c_str());
																	return false;
																}
																action =
																		new highlevel::ActionEndPoint(
																				endPoint,
																				string(
																						s_a_value));
															}
											...
											assert(action != NULL);
											for (list<GenericAction*>::iterator ga =
													genericActions.begin();
													ga != genericActions.end();
													ga++)
										...
										logger(ORCH_DEBUG_INFO, MODULE_NAME,
										__FILE__, __LINE__,
												"Invalid key \"%s\" in a rule of \"%s\"",
												name.c_str(), FLOW_RULES);
										return false;
									}
								}
								...
								if (!foundAction || !foundMatch || !foundID) {
									logger(ORCH_DEBUG_INFO, MODULE_NAME,
									__FILE__, __LINE__,
											"Key \"%s\", or key \"%s\", or key \"%s\", or all of them not found in an elmenet of \"%s\"",
											_ID, MATCH, ACTIONS, FLOW_RULES);
									return false;
								}

								highlevel::Rule rule(match, action, ruleID,
										priority);

								if (!graph.addRule(rule)) {
									logger(ORCH_DEBUG_INFO, MODULE_NAME,
									__FILE__, __LINE__,
											"The graph has at least two rules with the same ID: %s",
											ruleID.c_str());
									return false;
								}
							...
							for (list<highlevel::Rule>::iterator r =
									rules.begin(); r != rules.end(); r++) {
								list<highlevel::Rule>::iterator next_rule = r;
								next_rule++;
								if (next_rule != rules.end()) {
									uint64_t priority = (*r).getPriority();
									for (list<highlevel::Rule>::iterator r1 =
											next_rule; r1 != rules.end();
											r1++) {
							...
							if (same_priority) {
								logger(ORCH_WARNING, MODULE_NAME, __FILE__,
								__LINE__,
										"One or more flow rule with the same priority...");
								logger(ORCH_WARNING, MODULE_NAME, __FILE__,
								__LINE__,
										"Note that, if they match the same port, they may cause a conflict on the vSwitch!");
							}
						} catch (exception& e) {
							logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__,
							__LINE__,
				...
				if (!foundFlowRules) {
					logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__,
							"Key \"%s\" not found in \"%s\"", FLOW_RULES,
							FORWARDING_GRAPH);
					return false;
				}
		...
		if (!foundFlowGraph) {
			logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__,
					"Key \"%s\" not found", FORWARDING_GRAPH);
			return false;
		}
	} catch (exception& e) {
		logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__,
				"The content does not respect the JSON syntax: %s", e.what());
		return false;
	}
	...
	for (map<string, set<unsigned int> >::iterator it = nfs_ports_found.begin();
			it != nfs_ports_found.end(); it++) {
		set<unsigned int> ports = it->second;
		assert(ports.size() != 0);

		for (set<unsigned int>::iterator p = ports.begin(); p != ports.end();
				p++) {
		...
		logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__,
				"NF \"%s\" requires ports:", it->first.c_str());
		for (set<unsigned int>::iterator p = ports.begin(); p != ports.end();
				p++)
			logger(ORCH_DEBUG_INFO, MODULE_NAME, __FILE__, __LINE__, "\t%d",
					*p);
	}
