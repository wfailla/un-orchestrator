...
void print_ddbrokerkeys(ddbrokerkeys_t *keys) {
  ...
  printf("Cookie: \t%" PRIu64, keys->cookie);
  printf("Hash:\t\t%s",keys->hash);

  printf("Tenants:  %lu : ", zlist_size(keys->tenants));
  ...
  while (k) {
    ...
    printf("\t name: %s cookie %" PRIu64, ten->name, ten->cookie);
        ...
        json_object_object_foreach(val3, key4, val4) {
          ...
          if (customer_name && customer_pubkey) {
            ...
            retval = base64_decode_block(
                customer_pubkey, strlen(customer_pubkey), pubkey, &state_in);
